...
public class Service {
    ...
    /** Gửi trạng thái quest khi login hoặc cập nhật */
    public static void send_quest(Session conn) throws IOException {
        Quest q = QuestManager.getPlayerQuest(conn.p);
        if (q == null) return;
        if (!QuestManager.isQuestAccepted(conn.p)) {
            send_quest_info(conn, q);
        } else if (QuestManager.isQuestComplete(conn.p)) {
            send_quest_finish(conn, q);
        } else {
            send_quest_doing(conn, q, Math.max(0, conn.p.questKill));
        }
    }
...
//-----------------------------------lỗi ở đây----------------------------------
    ...
    //ChienNV
    /** Hiện hội thoại quest khi click NPC giao quest (dấu !) */
    public static void send_quest_info(Session conn, Quest q) throws IOException {
        Message m = new Message(52);
        m.writer().writeByte(0); // quest có thể nhận (!)
        m.writer().writeByte(1);
        m.writer().writeShort(q.id);
        m.writer().writeBoolean(q.isMain);
        m.writer().writeUTF(q.name);
        m.writer().writeByte(q.npcFrom);
        m.writer().writeUTF(q.desc);
        m.writer().writeByte(q.npcTo);
        m.writer().writeUTF(q.detail);
        conn.addmsg(m);
        m.cleanup();
    }

    /** Hiện hội thoại hoàn thành khi click NPC trả quest (dấu ? vàng) */
    public static void send_quest_finish(Session conn, Quest q) throws IOException {
        Message m = new Message(52);
        m.writer().writeByte(2); // hoàn thành (dấu ? vàng)
        m.writer().writeByte(1);
        m.writer().writeShort(q.id);
        m.writer().writeBoolean(q.isMain);
        m.writer().writeUTF(q.name);
        m.writer().writeByte(q.npcTo);
        m.writer().writeUTF("Ngươi đã hoàn thành nhiệm vụ " + q.name + "!");
        m.writer().writeUTF("Hãy nhận phần thưởng của ngươi.");
        conn.addmsg(m);
        m.cleanup();
    }

    /** Sau khi nhận quest - xóa ! và hiện ? trên NPC trả quest */
    public static void send_quest_doing(Session conn, Quest q, int currentKill) throws IOException {
        // 1. Xóa dấu ! (gửi byte=0, count=0)
        Message m0 = new Message(52);
        m0.writer().writeByte(0);
        m0.writer().writeByte(0);
        conn.addmsg(m0);
        m0.cleanup();
        // 2. Hiện dấu ? trên NPC trả quest (byte=1 format)
        Message m = new Message(52);
        m.writer().writeByte(1);
        m.writer().writeByte(1);
        m.writer().writeShort(q.id);

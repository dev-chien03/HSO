...
//                }
            ...
            case 52: {
                // Client gửi quest action, thực tế có thể khác format tùy bản client.
                // Hỗ trợ nhiều biến thể để không kẹt luồng nhận/trả nhiệm vụ.
                if (conn.p != null) {
                    try {
                        DataInputStream reader = m.reader();

                        int available = reader.available();
                        short questId = conn.p.questId;
                        byte action = 0;

                        if (available >= 3) {
                            reader.mark(8);
                            short q1 = reader.readShort();
                            byte a1 = reader.readByte();
                            template.Quest t1 = core.QuestManager.getById(q1);

                            if (t1 != null && (a1 == 0 || a1 == 1)) {
                                questId = q1;
                                action = a1;
                            } else {
                                reader.reset();
                                byte a2 = reader.readByte();
                                short q2 = reader.readShort();
                                template.Quest t2 = core.QuestManager.getById(q2);
                                if (t2 != null) {
                                    questId = q2;
                                    action = a2;
                                }
                            ...
                            short qOnly = reader.readShort();
                            if (core.QuestManager.getById(qOnly) != null) {
                                questId = qOnly;
                            }
                            // payload 2 byte không có action rõ ràng -> mặc định accept
                            action = 0;
                        }

                        // Chuẩn hóa action: 0=accept, còn lại xem như turn-in
                        if (action != 0) {
                            action = 1;
                        }

                        template.Quest q = core.QuestManager.getById(questId);
                        if (q == null) {
                            q = core.QuestManager.getPlayerQuest(conn.p);
                        }
                        if (q == null) {
                            System.out.println("[QUEST] no active quest for cmd=52");
                            break;
                        }

                        System.out.println("[QUEST] cmd=52 parsed questId=" + q.id + " action=" + action + " playerQuest=" + conn.p.questId + " bytes=" + available);

                        if (action == 0) {
                            // Nhận quest - chuyển dấu sang NPC trả quest
                            if (conn.p.questId == q.id) {
                                if (!core.QuestManager.isQuestAccepted(conn.p)) {
                                    conn.p.questKill = 0;
                                }
                                Service.send_quest_doing(conn, q, conn.p.questKill);
                            }
                        } else if (action == 1) {
                            // Trả quest
                            boolean canTurnIn = core.QuestManager.isQuestAccepted(conn.p)
                                    && (core.QuestManager.isQuestComplete(conn.p) || q.type == 0);
                            if (canTurnIn) {
                                core.QuestManager.completeQuest(conn.p);
                            } else if (q.type == 1) {
                                Service.send_notice_box(conn, "Chưa hoàn thành! " + conn.p.questKill + "/" + q.killCount);
                            }
                        }
                    } catch (Exception e) {
                        System.out.println("[QUEST] error reading cmd=52: " + e.getMessage());
                    }
                }
                break;
            }
    ...
    private void login(Message m) throws IOException {
        if (conn.p == null) {
            ...
            if (p0 != null && p0.setup()) {
                // Bảo vệ: chỉ cho phép đăng nhập nhân vật thuộc đúng tài khoản hiện tại
                boolean isOwnerCharacter = false;
                if (conn.list_char != null) {
                    for (String charName : conn.list_char) {
                        if (charName != null && !charName.isEmpty() && charName.equals(p0.name)) {
                            isOwnerCharacter = true;
                            break;
                        }
                    }
                }
                if (!isOwnerCharacter) {
                    System.out.println("[LOGIN] blocked cross-account character login: user=" + conn.user
                            + " playerId=" + id_player_login + " playerName=" + p0.name);
                    conn.close();
                    return;
                }

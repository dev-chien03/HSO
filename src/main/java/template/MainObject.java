...
//        }
        ...
        if (focus.hp <= 0) {
            if (focus.isPlayer() && ptHP > 70) {
                ...
                // }
                if (!focus.isMobDiBuon() && !focus.isPlayer() && focus.template != null && focus.template.mob_id >= 89 && focus.template.mob_id <= 92) {
                    p.update_point_arena(20);
                    Manager.gI().chatKTGprocess("@Server : " + p.name + " đã đánh sập " + focus.template.name);
                    ChienTruong.gI().update_house_die(focus.template.mob_id);
                }
       ...
       // ===== Mob tài nguyên bị đánh hoặc nhân bản bị đánh → nhân bản phản công =====
        // 1. Bạn đánh Nhân Bản → nhân bản phản công
        if (focus.isNhanBan() && ObjAtk.isPlayer() && !focus.isdie) {
            NhanBan nb = (NhanBan) focus;
            nb.p_target = (Player) ObjAtk;
            nb.is_move = false;
        }
        // 2. Bạn đánh Mỏ Tài Nguyên → nhân bản bảo vệ phải phản công
        if (focus.isMoTaiNguyen() && ObjAtk.isPlayer()) {
            Mob_MoTaiNguyen mo = (Mob_MoTaiNguyen) focus;
            if (mo.nhanban != null && !mo.nhanban.isdie) {
                mo.nhanban.p_target = (Player) ObjAtk;
                mo.nhanban.is_move = false;
            }
        }

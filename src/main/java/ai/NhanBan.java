...
public class NhanBan extends MainObject{
    ...
    public NhanBan(JSONArray jar) {
        map_id = (byte) toLong(jar.get(0));
        index  = (int) toLong(jar.get(1));
        x      = (short) toLong(jar.get(2));
        y      = (short) toLong(jar.get(3));
        name   = (String) jar.get(4);
        part_p = new ArrayList<>();
        JSONArray jar2 = (JSONArray) jar.get(5);
        if (jar2 != null && !jar2.isEmpty()) {
            for (int i = 0; i < jar2.size(); i++) {
                JSONArray jar3 = (JSONArray) jar2.get(i);
                if (jar3 == null || jar3.isEmpty()) continue;
                Part_player pr = new Part_player();
                pr.type = (byte) toLong(jar3.get(0));
                pr.part = (byte) toLong(jar3.get(1));
                part_p.add(pr);
            }
        }
        clazz  = (byte) toLong(jar.get(6));
        head   = (byte) toLong(jar.get(7));
        eye    = (byte) toLong(jar.get(8));
        hair   = (byte) toLong(jar.get(9));
        level  = (short) toLong(jar.get(10));
        hp     = (int) toLong(jar.get(11));
        hp_max = (int) toLong(jar.get(12));
        pointpk = (short) toLong(jar.get(13));
        clan_icon = (short) toLong(jar.get(14));
        clan_id   = (int) toLong(jar.get(15));
        clan_name_clan_shorted = (String) jar.get(16);
        clan_mem_type = (byte) toLong(jar.get(17));

        jar2 = (JSONArray) jar.get(18);
        if (jar2 != null) {
            fashion = new byte[jar2.size()];
            for (int i = 0; i < jar2.size(); i++) {
                fashion[i] = (byte) toLong(jar2.get(i));
            }
        }
        mat_na   = (short) toLong(jar.get(19));
        phi_phong = (short) toLong(jar.get(20));
        weapon   = (short) toLong(jar.get(21));
        id_horse = (short) toLong(jar.get(22));
        id_hair  = (short) toLong(jar.get(23));
        id_wing  = (short) toLong(jar.get(24));

        danh_hieu = (short) toLong(jar.get(25));
        type_use_mount = (byte) toLong(jar.get(26));
        dame           = (int) toLong(jar.get(27));
        act_time       = toLong(jar.get(28));
        is_move        = toBool(jar.get(29));
        p_skill_id     = (int) toLong(jar.get(30));
        crit           = (int) toLong(jar.get(31));
        time_hp_buff   = toLong(jar.get(32));
        def            = (int) toLong(jar.get(33));
        pierce         = (int) toLong(jar.get(34));
    }
    ...
    public void update(Map map) {
        try {
            if (!isdie && p_target != null && !p_target.isdie) {
                if (Math.abs(x - p_target.x) > 40) {
                    if (x < p_target.x) x += 10;
                    else x -= 10;
                }
                if (Math.abs(x - p_target.x) <= 40) {
                    MainObject.MainAttack(map, this, p_target, 0, null, 0);
                }
            }
        } catch (Exception e) {}
    }
